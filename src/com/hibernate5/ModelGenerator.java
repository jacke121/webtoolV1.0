	...
	private void generate(String tableName, List<Entry<String, String>> colums) throws Exception {
		...
		for (int i = 0; i < colums.size(); i++) {
			...
			if (entry.getValue().equals("Date")) {
					haveDate = "import java.util.Date;";
			}
			if(entry.getValue().equals("BigDecimal")){
		...
		String tmpstr=sb.toString().replace("@@haveDate", haveDate);
		tmpstr=tmpstr.replace("@@bigDecimal", bigDecimal);
		bos.write(tmpstr.getBytes());
		bos.close();
	}
